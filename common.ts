...
import { readableStreamToString, HsbcAccountInformationCeError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new HsbcAccountInformationCeError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new HsbcAccountInformationCeError(e, e.response?.data)
                }
                if (innerError instanceof HsbcAccountInformationCeError) {
                    // Got "HsbcAccountInformationCeError" from the above try block
                    throw innerError;
                }
